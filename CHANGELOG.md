...
- **Seven high-severity CVEs in transitive dependencies.** `mailparser` moves to
  ...
  the exact version that later became vulnerable, so it's now a range. Prisma's
  own copy of `deepmerge-ts` is pinned separately via a scoped override, since
  `@prisma/config` depends on exactly 7.1.5.
