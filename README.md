...
- In-app feedback from Help & FAQ — report a bug or suggest an enhancement without leaving the app; files a labeled issue on the project's GitHub repo (`GITHUB_FEEDBACK_TOKEN`; no repository config needed unless you're running a fork)
...
| --- | --- |
| ![Settings — security log](docs/screenshots/settings-security.png) |  |
...
| `GITHUB_FEEDBACK_TOKEN` | Optional. Set to enable the feedback form on Help & FAQ, which files submissions as issues on `jaysbeekay/Hearth`. The token needs Issues: Read and write permission for that repository; configure the repository's GitHub Project automation to auto-add new issues and place them in the review column. |
| `GITHUB_FEEDBACK_REPOSITORY` | Optional. Overrides which `owner/repository` feedback is filed against — only needed for a fork that wants its own issues repo instead of `jaysbeekay/Hearth`. |
