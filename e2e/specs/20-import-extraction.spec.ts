...
test.describe("bulk document import and extraction", () => {
  ...
  test("wealth CSV trade import page exists and accepts CSV files", async ({ page }) => {
    ...
    await page.goto(`/wealth/${portfolioId}/import`);

    // Verify the page exists and has file input
    await expect(page.locator('input[type="file"]')).toBeTruthy();

    // Create a minimal CSV with trade data
    const csvData = "symbol,date,type,quantity,price\nAAPL,2026-01-01,BUY,10,150";
    ...
    // The page should handle the CSV upload
    const pageContent = await page.locator("body").innerText();
    expect(
      pageContent.includes("import") ||
  ...
  test("import page is accessible from main navigation", async ({ page }) => {
    await page.goto("/");

    // Look for import link in navigation
    const importLink = page.locator('a:has-text("Import")');
